---
title: 'Push Notifications'
subtitle: "The site is experiencing higher than normal traffic and we have temporarily halted database creation."
---

## Is my application still connected to my ChattyCloud database?
Absolutely! You just can’t create any new branches or databases at this time. All existing databases and branches are still deployed. Additionally, you can still make changes to existing developer branches and deploy them using the deploy request feature.

![dashboard-firebase-credentials](/img/docs/dashboard-fcm-credentials.png)

## Does this affect the data in my databases?
No, the databases you have deployed in ChattyCloud, and any data they contain, are not impacted by this change.

Only branch and database creation has been temporarily halted. We apologize for the inconveniance and we're working to get things ready again. 

## How will I know when it's safe to create new branches or databases again?
When you return to our site, the banner mentioning _"temporarily halted database creation"_ will no longer be there.

You can also check out our [status page](http://chatty-cloud.freshstatus.io/).

## Need help?
Get help from [ChattyCloud's support team](https://www.chatty-cloud.com/support), or join our [GitHub Discussion board](https://github.com/chatty-cloud/beta/discussions) to see how others are using ChattyCloud.
 
<NextBlock
  steps={[
    {
      text: 'Group Chat',
      link: '/getting-started/group-chat',
    },
  ]}
></NextBlock>
