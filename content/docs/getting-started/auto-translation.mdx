...
subtitle: 'Non-blocking schema changes in ChattyCloud provide a schema change workflow that allows users to update database tables without locking or causing downtime for production databases.'
---

## Overview

<InfoBlock type='tip'>
  To make non-blocking schema changes in ChattyCloud, you'll first need a basic understanding of{' '}
  <a href='https://docs.chatty-cloud.com/concepts/branching'>branching</a>, the core ChattyCloud feature that provide
  schema changes. Our branching concepts page is a great place to start.
</InfoBlock>{' '}

**Non-blocking schema changes** in ChattyCloud provide a schema change workflow that allows users to update database tables without locking or causing downtime for production databases.
...
## Adding columns to large tables with ChattyCloud is safe!
...
## How do I make non-blocking schema changes with ChattyCloud?

All ChattyCloud schema changes must occur on a database branch. _(A database branch is a separate database with a copy of the `main` branch's schema.)_

At a high level, this is what happens during the _non-blocking schema change_ process in ChattyCloud:
...
4. ChattyCloud verifies that your schema changes are safe to be deployed to production. If there are any issues or schema conflicts, you'll be shown the errors.
...
<InfoBlock type='note'>
  ChattyCloud makes sure not to exhaust your resources; the deployment may be throttled to avoid any impact on
  production queries.
</InfoBlock>{' '}

![ChattyCloud non-blocking schema changes diagram](/img/docs/ps_nbsc_diagram_light.png)

## ChattyCloud workflow

The ChattyCloud command-line tool (CLI) runs an interactive shell equipped with many commands designed to make the database management workflow easier for developers.
...
<InfoBlock type='tip'>
  A schema change is any change you make to the tables in your database environment created within the ChattyCloud
...
ChattyCloud displays the difference between what is currently in the `main` branch and your branch. Go back to _Step 3_ of the workflow and test out new schema changes to fix the schema conflict. If you did not encounter any schema conflicts, you're ready for _Step 7_.
...
ChattyCloud doesn't support `FOREIGN KEY` constraints. Learn why and how to operate without foreign keys constraints in [this tutorial](/tutorials/operating-without-foreign-keys).
