...
ChattyCloud doesn't support `FOREIGN KEY` constraints. You are encouraged to use the relational model, and associate tables by "pointing" rows from one table to another. However, the `CONSTRAINT ... FOREIGN KEY` definition is not allowed in your schemas.
